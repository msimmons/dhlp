...
public class DHLPController {
   ...
   */
   public void shutdown() {
      for ( DHLPConnectionPool pool : pools.values() ) {
         pool.close();
      }
      pools.clear();
      connections.clear();
   ...
   */
   public SortedMap<String,JDBCObject> findObjects(String connection, boolean refresh) {
      ...
      try {
         ...
         ResultSet results = db.getMetaData().getTables(pool.getCatalog(), pool.getSchema(), "%", null);
         while ( results.next() ) {
         ...
         results.close();
         // Procedures
         results = db.getMetaData().getProcedures(pool.getCatalog(), pool.getSchema(), "%");
         while ( results.next() ) {
            JDBCObject object = JDBCObject.create(results);
            objects.put(object.getDisplayName(), object);
         }
   ...
   */
   public int getFetchLimit() { return ( dhlpData == null ) ? 0 : dhlpData.getFetchLimit(); } 
